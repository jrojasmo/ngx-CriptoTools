...
export class DesComponent implements AfterViewInit{
  ...
  cifrar(mode) {
    ...
    password = (this.key_generator(password,this.P48,this.P56,this.shift_table));
    ...
    switch (mode) {
      case 'ecb':
        this.CipherDES_image_ECB(pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
      case 'cbc':
        this.CipherDES_image_CBC(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
      case 'ofb':
        this.CipherDES_image_OFB(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX)
        break;
      case 'cfb':
        this.CipherDES_image_CFB(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX)
        break;
      case 'ctr':
        this.CipherDES_image_CTR(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
    }
  ...
  descifrar(mode) {
    ...
    switch (mode) {
      case 'ecb':
        this.DecipherDES_image_ECB(pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
      case 'cbc':
        this.DecipherDES_image_CBC(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
      case 'ofb':
        this.DecipherDES_image_OFB(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX)
        break;
      case 'cfb':
        this.DecipherDES_image_CFB(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX)
        break;
      case 'ctr':
        this.DecipherDES_image_CTR(iv, pix, this.P32, password, this.IPDES, this.IPIDES, this.E_PDES, this.S8BOX);
        break;
    }
  ...
  showClaveIncorrecta() {
    this.dialogService.open(DialogoComponent, {
      context: {
        ...
        content: 'La clave debe ser un número hexadecimal de 16 dígitos'
      },
    });
  }
...
CipherDES_ECB(PlainText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  //DES CIPHER
  // RETURN CIPHER TEXT
...
DecipherDES_ECB(CipherText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // DES DECIPHER
  // RETURN PLANE TEXT
  var decipher_text = this.CipherDES_ECB(CipherText, P32, Keys_t[1], IPDES, IPIDES, E_PDES, S8BOX);
  return decipher_text;
}
CipherDES_image_ECB(scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var cipher_pixels = this.CipherDES_ECB(Pl_txt_hexa, P32, Keys_t[0], IPDES, IPIDES, E_PDES, S8BOX);
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.DecipherDES_ECB(Pl_txt_hexa, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
              var ci_pi2 = this.array_f(cipher_pixels);
              for (let j = 0; j < 8; j++) {
                  var color_c1 = ci_pi2.slice(j*8,(j*8)+8);
                  var color_c = color_c1.join('');
...
CipherDES_CBC(IV, PlainText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  //DES CIPHER
  // RETURN CIPHER TEXT
  var IV_bin = this.array_f(IV);
  var PlainText2 = this.array_f(PlainText);
  var PlainText2 = this.xor_bin(IV_bin,PlainText2);
  let cipher_text = [];
  let PT_IP64 = [];
  let m_left_pt=[];
  let m_right_pt=[];
...
DecipherDES_CBC(IV,CipherText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // DES DECIPHER
  // RETURN CIPHER TEXT
  var IV_bin = this.array_f(IV);
  var PlainText2 = this.array_f(CipherText);
  Keys_t = Keys_t[1];
  let cipher_text = [];
  let PT_IP64 = [];
  let m_left_pt=[];
  ...
  var pre_cipher_text = left_pt.concat(right_pt);
  cipher_text= this.permute(IPIDES,pre_cipher_text,cipher_text);
  cipher_text = this.xor_bin(IV_bin,cipher_text);
  var cipher_text_hexa = this.bin2hex2(cipher_text.join(''));
  return cipher_text_hexa;
}

CipherDES_image_CBC(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // CIPHER THE IMAGE WITH INFO OF RGBT IN "scannedData"
  // RETURN NEW ARRAY OF CIPHER RGBT
  var N_IV = (IV);
  var sup_sd = 0;
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.CipherDES_CBC(N_IV, Pl_txt_hexa, P32, Keys_t[0], IPDES, IPIDES, E_PDES, S8BOX);
              N_IV = cipher_pixels;
              var ci_pi2 = this.array_f(cipher_pixels);
              for (let j = 0; j < 8; j++) {
                  var color_c1 = ci_pi2.slice(j*8,(j*8)+8);
  ...
  for (let i = 0; i <= scannedData.length-10 ; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.DecipherDES_CBC(N_IV, Pl_txt_hexa, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
              N_IV = Pl_txt_hexa;
              var ci_pi2 = this.array_f(cipher_pixels);
              for (let j = 0; j < 8; j++) {
                  var color_c1 = ci_pi2.slice(j*8,(j*8)+8);
...
CipherDES_CFB(IV, PlainText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  var IV_bin = this.array_f(IV);
  var PlainText2 = this.array_f(PlainText);
  let cipher_text = [];
  let PT_IP64 = [];
  ...
  var left_pt = [];
  var right_pt = [];
  PT_IP64 = this.permute(IPDES, IV_bin, PT_IP64);
  left_pt = PT_IP64.slice(0,32);
  right_pt = PT_IP64.slice(32,65);
  for (let nr = 0; nr < 16; nr++){
      let EP_48 = [];
  ...
  var pre_cipher_text = left_pt.concat(right_pt);
  cipher_text = this.permute(IPIDES,pre_cipher_text,cipher_text);
  cipher_text = this.xor_bin(cipher_text,PlainText2);
  var cipher_text_hexa = this.bin2hex2(cipher_text.join(''));
  return cipher_text_hexa;
}

CipherDES_image_CFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // CIPHER THE IMAGE WITH INFO OF RGBT IN "scannedData"
  // RETURN NEW ARRAY OF CIPHER RGBT
  var N_IV = (IV);
  var sup_sd = 0;
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.CipherDES_CFB(N_IV, Pl_txt_hexa, P32, Keys_t[0], IPDES, IPIDES, E_PDES, S8BOX);
              N_IV = cipher_pixels;
              var ci_pi2 = this.array_f(cipher_pixels);
              for (let j = 0; j < 8; j++) {
                  var color_c1 = ci_pi2.slice(j*8,(j*8)+8);
...
CipherDES_OFB(IV, PlainText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  //DES CIPHER
  // RETURN CIPHER TEXT
  // console.log("IV1",IV);
  var IV_bin = this.array_f(IV);
  ...
  var pre_cipher_text = left_pt.concat(right_pt);
  cipher_text=this.permute(IPIDES,pre_cipher_text,cipher_text);
  var final_xor_ofb = this.xor_bin(PlainText2,cipher_text);
  var cipher_text_hexa = this.bin2hex2(cipher_text.join(''));
  var f_xor_hexa = this.bin2hex2(final_xor_ofb.join(''));
  var new_output = [f_xor_hexa,cipher_text_hexa]; //cipher text & new IV  
  return new_output;
}
CipherDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // CIPHER THE IMAGE WITH INFO OF RGBT IN "scannedData"
  // RETURN NEW ARRAY OF CIPHER RGBT
  var N_IV = (IV);
  var sup_sd = 0;
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.CipherDES_OFB(N_IV, Pl_txt_hexa, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
              N_IV = cipher_pixels[1];
              var ci_pi2 = this.array_f(cipher_pixels[0]);
              for (let j = 0; j < 8; j++) {
                  var color_c1 = ci_pi2.slice(j*8,(j*8)+8);
                  var color_c = color_c1.join('');
                  var color_c_d = this.convertNumber(color_c,2,10);
...
DecipherDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  scannedData = this.CipherDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
  return scannedData;
}
//------------------------------------------ CTR -----------------------------------------------------------

CipherDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
  // CIPHER THE IMAGE WITH INFO OF RGBT IN "scannedData"
  // RETURN NEW ARRAY OF CIPHER RGBT
  var sup_sd = 0;
  var cont = 0;
  ...
  for (let i = 0; i <=scannedData.length-10; i+=1) {
      if (!((i+1)%4===0)){
          ...
          if (cont2===8) {
              ...
              var Pl_txt_hexa = this.bin2hex2(Pl_txt.join(''));
              var cipher_pixels = this.CipherDES_ECB(IV, P32, Keys_t[0], IPDES, IPIDES, E_PDES, S8BOX);
              var IV = this.CTR_plus(IV);
              var ci_pi2 = this.array_f(cipher_pixels);
...
DecipherDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
  scannedData = this.CipherDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX)
  return scannedData;
}
